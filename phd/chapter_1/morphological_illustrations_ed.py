...
for i in range(2):
    ...
    fill_circle = axs[i].fill_between(x_circle, f_minus, f_plus, color='white')
    line_circle = axs[i].add_patch(m_patches.Circle((2, 0), r_in, fill=False, color='tab:blue', lw=1.5))

circle_center = (1, -1.2)
r_out = d / 1.5
for i in range(2):
    axs[i].add_patch(m_patches.Circle(circle_center, r_out, fill=False, color='tab:blue', lw=1.5))
    axs[i].add_patch(m_patches.Circle(circle_center, r_out, color='tab:blue', lw=0, alpha=alpha))
...
# Erosion
interior_points = get_interior_points(points, d)

interior_upper_points = interior_points[:, :n * 2]
interior_lower_points = interior_points[:, n * 2:]

x = np.linspace(interior_upper_points[0, 0], interior_upper_points[0, n * 2 - 1], 2 * n)
y_l = np.interp(x, np.flip(interior_lower_points[0, :]), np.flip(interior_lower_points[1, :]))
y_u = np.interp(x, interior_upper_points[0, :], interior_upper_points[1, :])

line_int = axs[0].plot(interior_points[0, :], interior_points[1, :], '--', color=colour_dilation)
fill_int = axs[0].fill_between(x, y_l, y_u, alpha=alpha, color=colour_dilation)

axs[0].add_patch(m_patches.Circle((2, 0), r_in + d, color='white', lw=0))
axs[0].add_patch(m_patches.Circle((2, 0), r_in + d, color='tab:blue', lw=0, alpha=alpha))
axs[0].add_patch(m_patches.Circle((2, 0), r_in + d, fill=False, color=colour_dilation, lw=1.5, linestyle='--'))

axs[0].fill_between(x_circle, f_minus, f_plus, color='white')
axs[0].add_patch(m_patches.Circle((2, 0), r_in, fill=False, color='tab:blue', lw=1.5))

# Dilation
exterior_points = get_exterior_points(points, d)

exterior_upper_points = exterior_points[:, :n * 2]
exterior_lower_points = exterior_points[:, n * 2:]

x = np.linspace(exterior_upper_points[0, 0], exterior_upper_points[0, n * 2 - 1], 2 * n)
y_l = np.interp(x, np.flip(exterior_lower_points[0, :]), np.flip(exterior_lower_points[1, :]))
y_u = np.interp(x, exterior_upper_points[0, :], exterior_upper_points[1, :])

line_ext, = axs[1].plot(exterior_points[0, :], exterior_points[1, :], '--', color=colour_dilation)
fill_ext = axs[1].fill_between(x, y_l, y_u, alpha=alpha, color=colour_dilation)

axs[1].add_patch(m_patches.Circle(circle_center, r_out + d, fill=False, color=colour_dilation, lw=1.5, linestyle='--'))
axs[1].add_patch(m_patches.Circle(circle_center, r_out + d, color=colour_dilation, lw=0, alpha=alpha))
...
fig_1.tight_layout()
fig_2.tight_layout()
fig_1.savefig('erosion.svg', transparent=True)
fig_2.savefig('dilation.svg', transparent=True)
plt.show()
