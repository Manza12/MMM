...
edge_labels = {k: '$%s$' % edge_labels[k] for k in edge_labels.keys()}
nx.draw_networkx_edge_labels(graph, pos, edge_labels=edge_labels, label_pos=0.65)
...
graph_1 = graph.derive_label_weight(placement=0.3)
graph_1.add_node('SS', pos=(-0.5, 0), label='$SS$')
graph_1.add_node('EE', pos=(6, 0), label='$EE$')
graph_1.add_edge('SS', (1, 2), weight=1)
graph_1.add_edge('SS', (1, 3), weight=1)
graph_1.add_edge((7, 8), 'EE', weight=0)
graph_1.edges[(6, 7), (7, 8)]['weight'] = 0

plt.figure(figsize=(6., 2.))

pos = nx.get_node_attributes(graph_1, 'pos')
nx.draw_networkx_edges(graph_1, pos, arrows=True, node_size=800)
...
edge_labels = {k: '$%s$' % edge_labels[k] for k in edge_labels.keys()}
nx.draw_networkx_edge_labels(graph_1, pos, edge_labels=edge_labels, label_pos=0.55)

plt.axis('off')
plt.tight_layout()

file_path = figures_path / (name + '_1.pdf')
plt.savefig(file_path)
